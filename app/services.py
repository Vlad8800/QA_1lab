...
# ==========================================
# Порушення для Завдання 3 (Симуляція FAIL)
# ==========================================


def broken_tax_calculator(price: float) -> float:
    """Порушення 1: ZeroDivisionError (Reliability / Blocker Issue)."""
    zero_val = 0
    return price / zero_val


def duplicate_shipping_rate(
    country_code: str,
    weight: float,
    is_express: bool,
    customer_tier: str,
    ...
    """Порушення 2: Надлишкове дублювання логіки (Duplicated Lines > 3.0%)."""
    if country_code.upper() == "UA":
        return 2.0
