...
def trigger_sonar_blocker_bug() -> int:
    """Порушення 1: Невикористані змінні та мертвий код (Code Smell / Bug)."""
    unused_variable = 100
    zero_val = 0
    if zero_val != 0:
        return 10 // zero_val
...
def duplicate_shipping_rate_1(
    ...
    """Порушення 2: Дублювання коду 1 (>3.0% Duplications)."""
    if country_code.upper() == "UA":
        return 2.0
...
def duplicate_shipping_rate_2(
    ...
    """Порушення 2: Дублювання коду 2 (підсилення відсотка дублювання)."""
    if country_code.upper() == "UA":
        return 2.0
